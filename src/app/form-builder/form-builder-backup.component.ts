...
export class FormBuilderComponent implements AfterViewInit {
  ...
  addTextFieldToSection(innerGridId: string): void {
    ...
    // Step 2: Append off-DOM for measurement
    fieldItem.style.visibility = 'hidden';
    fieldItem.style.position = 'absolute';
    document.body.appendChild(fieldItem);

    // Step 3: Wait for render, then measure and set correct h
    requestAnimationFrame(() => {
      const elementHeight = fieldItem.offsetHeight;
      const rowHeight = innerGrid.getCellHeight(); // GridStack cell height in px

      const dynamicH = Math.ceil(elementHeight / rowHeight);

      // Set actual values
      fieldItem.setAttribute('gs-h', dynamicH.toString());
      fieldItem.style.visibility = '';
      fieldItem.style.position = '';

      innerGrid.el.appendChild(fieldItem);
      innerGrid.makeWidget(fieldItem);

      // Optional: Apply the new size via GridStack API
      innerGrid.update(fieldItem, {
        x: nextX,
        y: nextY,
        w: fieldWidth,
        h: dynamicH,
      ...
      // Clean up from temp append
      if (fieldItem.parentElement === document.body) {
        document.body.removeChild(fieldItem);
      }
